...
func SearchJav(ctx context.Context, actors []string, tagIDs []int64, search, sort string, limit, offset int, seed *int64) ([]models.Jav, int64, error) {
	...
	case "play_count", "play_count_desc":
		order = "COALESCE((SELECT SUM(COALESCE(v.play_count, 0)) FROM video_location vl JOIN directory d ON d.id = vl.directory_id JOIN video v ON v.id = vl.video_id WHERE vl.jav_id = jav.id AND " + activeLocationWhereSQL("vl", "d") + "), 0) DESC, jav.created_at DESC, jav.id DESC"
	case "play_count_asc":
		order = "COALESCE((SELECT SUM(COALESCE(v.play_count, 0)) FROM video_location vl JOIN directory d ON d.id = vl.directory_id JOIN video v ON v.id = vl.video_id WHERE vl.jav_id = jav.id AND " + activeLocationWhereSQL("vl", "d") + "), 0) ASC, jav.created_at ASC, jav.id ASC"
	case "recent_asc":
	...
	if err := attachJavLocationVideos(ctx, items); err != nil {
		return nil, 0, err
	}
	return items, total, nil
}

func attachJavLocationVideos(ctx context.Context, items []models.Jav) error {
	if len(items) == 0 {
		return nil
	}
...
func ListVideosForJavScan(ctx context.Context) ([]JavScanVideo, error) {
	...
	if err := common.DB.WithContext(ctx).
		...
		Select("vl.id AS location_id, vl.video_id, vl.relative_path AS filename, vl.jav_id, vl.updated_at, v.duration_sec, COALESCE(jav.provider, 0) AS jav_provider").
