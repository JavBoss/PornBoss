...
func ListVideos(ctx context.Context, limit, offset int, tagNames []string, search, sort string, seed *int64) ([]models.Video, error) {
	...
	case "filename", "filename_asc":
		orderClause = "video_location.relative_path COLLATE NOCASE, video_location.id"
	case "filename_desc":
		orderClause = "video_location.relative_path COLLATE NOCASE DESC, video_location.id DESC"
	...
	if cleanedSearch != "" {
		...
		query = query.Where("video_location.relative_path LIKE ? COLLATE NOCASE", like)
	}
...
func CountVideos(ctx context.Context, tagNames []string, search string) (int64, error) {
	...
	if len(cleanedTags) == 0 {
		base := common.DB.WithContext(ctx).
			...
			Where(activeLocationWhereSQL("video_location", "directory"))
		if like != "" {
			base = base.Where("video_location.relative_path LIKE ? COLLATE NOCASE", like)
		}
		var count int64
	...
	if like != "" {
		sub = sub.Where("video_location.relative_path LIKE ? COLLATE NOCASE", like)
	}
